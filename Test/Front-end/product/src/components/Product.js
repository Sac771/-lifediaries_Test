...
function Product() {
  ...
  function filter(ProductCategory){
    ...
    if(Categorydata[0]===undefined){
      ...
      var finalarr=[]
      for(let i=0;i<filteredCatData.length;i++){
        finalarr=[...finalarr,filteredCatData[i]['Category_Name']+'  ']
      }
      return finalarr
    }
  }
  ...
  return (
    ...
    Search by Category Name : <input type="text" onChange={(e)=>{filterrr(e.target.value)}} placeholder="Search.."></input>
    </div>
    <button type="button" className='btn-dark bg-dark text-white'><Link to="/add_product">Add New</Link></button>
